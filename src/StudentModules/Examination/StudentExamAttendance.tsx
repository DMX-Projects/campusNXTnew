...
export default function ExamAttendanceTable() {
  ...
  const downloadCsv = () => {
    if (filteredSubjects.length === 0 || filteredExams.length === 0) {
      alert("No attendance data to download.");
      return;
    }
    let csv =
      "Subject Code,Subject Name,Exam Type,Exam Date,Session,Semester,Attendance\n";
    filteredSubjects.forEach((subject) => {
      filteredExams.forEach((exam) => {
        const status = attendanceMap.get(`${exam.examId}-${subject.subjectCode}`) || "-";
        const row = [
          subject.subjectCode,
          subject.subjectName,
          exam.examName,
    ...
    a.href = url;
    a.download = "exam_attendance_report.csv";
    a.click();
      ...
      <section>
        <button
          onClick={() => {
            ...
            let csv =
              "Subject Code,Subject Name,Exam Type,Date of Exam,Session,Semester,Attendance Status\n";
            filteredSubjects.forEach((subject) => {
              filteredExams.forEach((exam) => {
                const status = attendanceMap.get(`${exam.examId}-${subject.subjectCode}`) || "-";
                const row = [
          ...
          className="mt-4 flex items-center gap-2 bg-indigo-600 hover:bg-indigo-700 text-white font-semibold px-5 py-3 rounded-md focus:outline-none focus:ring-2 focus:ring-indigo-400 transition"
          aria-label="Download attendance report CSV"
        >
