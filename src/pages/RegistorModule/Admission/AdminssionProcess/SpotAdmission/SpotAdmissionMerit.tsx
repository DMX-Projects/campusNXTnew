...
import { Search, Users, BookOpen, Layers, PlusCircle, UploadCloud, Send, X, Download, Eye } from 'lucide-react';
...
// --- MOCK DATA ---
const initialMeritCandidates: MeritCandidate[] = [
    { id: 'SPOT-M01', name: 'Zoya Akhtar', avatar: 'ZA', course: 'B.Tech AI & ML', category: 'General', score: 98.5 },
    { id: 'SPOT-M02', name: 'Vihaan Reddy', avatar: 'VR', course: 'B.Tech AI & ML', category: 'OBC', score: 97.9 },
    { id: 'SPOT-M03', name: 'Anaya Sharma', avatar: 'AS', course: 'B.Tech CSE', category: 'General', score: 97.5 },
    { id: 'SPOT-M04', name: 'Dev Singh', avatar: 'DS', course: 'B.Tech Cyber Security', category: 'SC', score: 96.8 },
    { id: 'SPOT-M05', name: 'Samaira Rao', avatar: 'SR', course: 'B.Tech CSE', category: 'General', score: 96.2 },
];

// --- MAIN COMPONENT ---
const MeritBasedAdmission = () => {
    const [candidates, setCandidates] = useState<MeritCandidate[]>(initialMeritCandidates);
    const [searchTerm, setSearchTerm] = useState('');
    const [isAddStudentModalOpen, setIsAddStudentModalOpen] = useState(false);
    const [isImportModalOpen, setIsImportModalOpen] = useState(false);
    const [newStudent, setNewStudent] = useState<Omit<MeritCandidate, 'avatar'>>({ id: '', name: '', course: '', category: 'General', score: 0 });
    ...
    const kpiStats = useMemo(() => [
        { title: 'Total Merit Seats', value: 25, icon: 'Users', color: 'text-indigo-500', bgColor: 'bg-indigo-100 dark:bg-indigo-900/50' },
        { title: 'Applications Received', value: candidates.length, icon: 'Layers', color: 'text-sky-500', bgColor: 'bg-sky-100 dark:bg-sky-900/50' },
        { title: 'High-Demand Courses', value: new Set(candidates.map(c => c.course)).size, icon: 'BookOpen', color: 'text-amber-500', bgColor: 'bg-amber-100 dark:bg-amber-900/50' },
    ], [candidates]);
    ...
    const handleDownloadSample = () => {
        const csvContent = "data:text/csv;charset=utf-8," + "id,name,course,category,score\n" + "SPOT-M99,Sample Student,B.Tech AI & ML,General,99.5\n";
        const encodedUri = encodeURI(csvContent);
        const link = document.createElement("a");
        link.setAttribute("href", encodedUri);
        link.setAttribute("download", "sample_merit_list.csv");
    ...
    const handleAddStudent = (e: React.FormEvent) => {
        e.preventDefault();
        const newCandidate: MeritCandidate = {
            ...newStudent,
            avatar: newStudent.name.split(' ').map(n => n[0]).join('').toUpperCase()
        };
        setCandidates(prev => [newCandidate, ...prev].sort((a, b) => b.score - a.score));
        setIsAddStudentModalOpen(false);
        setToast({ message: `Student ${newStudent.name} added successfully.`, type: 'success' });
        setNewStudent({ id: '', name: '', course: '', category: 'General', score: 0 });
    };
    ...
    return (
        <div className="min-h-screen bg-gray-50 dark:bg-gray-900 text-gray-800 dark:text-gray-200 p-4 sm:p-6 lg:p-8 font-sans">
            <div className="max-w-7xl mx-auto">
                ...
                <header className="mb-8"><h1 className="text-3xl font-bold tracking-tight text-gray-900 dark:text-white">Merit-Based Spot Admission</h1><p className="text-gray-500 dark:text-gray-400 mt-1">Manage admissions for high-demand courses based on merit scores.</p></header>
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">{kpiStats.map((item, index) => { const IconComponent: { [key: string]: ElementType } = { Users, BookOpen, Layers }; const Icon = IconComponent[item.icon]; return (<div key={index} className="bg-white dark:bg-gray-800 rounded-2xl shadow-sm p-6 transform hover:-translate-y-1 transition-transform duration-300"><div className="flex items-start justify-between"><div className={`p-3 rounded-lg ${item.bgColor}`}><Icon className={`w-6 h-6 ${item.color}`} /></div><p className="text-3xl font-bold text-gray-900 dark:text-white">{item.value}</p></div><div className="mt-4"><p className="text-gray-500 dark:text-gray-400 text-sm font-medium">{item.title}</p></div></div>);})}</div>
