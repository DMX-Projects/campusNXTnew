...
const PlacementCalendar: React.FC = () => {
  ...
  const [events, setEvents] = useState<PlacementEvent[]>([
    { date: `${today.getFullYear()}-${(today.getMonth()+1).toString().padStart(2,'0')}-05`, title: "Tech Innovators Drive", description: "On-campus placement drive" },
    { date: `${today.getFullYear()}-${(today.getMonth()+1).toString().padStart(2,'0')}-12`, title: "HR Round Prep", description: "Resume & interview prep session" },
    { date: `${today.getFullYear()}-${(today.getMonth()+1).toString().padStart(2,'0')}-18`, title: "Company Talk", description: "Info session by ABC Corp." },
  ]);
  ...
  const addEvent = () => {
    ...
    setEvents([...events, { date: selectedDate!, title: eventData.title, description: eventData.description }]);
    setShowModal(false);
  };
    ...
    >
      <span
        className={`font-semibold mb-1 ${
          isToday ? 'text-white bg-blue-500 rounded-full w-6 h-6 flex items-center justify-center' : ''
        }`}
      >
        {day.getDate()}
      </span>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center z-50">
          <div className="bg-white p-6 rounded shadow-lg w-1/3 space-y-4">
            ...
            <div className="flex justify-end space-x-2">
              ...
              <button onClick={addEvent} className="px-4 py-2 bg-blue-500 text-white rounded hover:bg-blue-600">Add Event</button>
            </div>
          </div>
        </div>
      )}
