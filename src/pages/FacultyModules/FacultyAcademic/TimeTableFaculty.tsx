...
import { Coffee, Clock, User, Calendar, Filter, BookOpen } from 'lucide-react';
...
const MOCK_TIMETABLE_DATA = {
    ...
    allFaculties: [
        ...
        { id: 'FAC02', name: 'Dr. Rajesh Kumar', expertise: ['SUB002', 'SUB005'] },
...
const FacultyView = ({ user, data }) => {
    const [filters, setFilters] = useState({ day: '', section: '' });
    const facultyId = user?.details?.facultyId;

    const { baseFacultyTimetable, taughtSections, uniqueSubjects } = useMemo(() => {
        const timetable = {};
        const sectionsSet = new Map();
        const subjectsSet = new Set();
        const allSections = flattenAcademicData(data.programs).sections;

        Object.entries(data.timetables).forEach(([sectionId, sectionTimetable]) => {
            Object.entries(sectionTimetable).forEach(([key, slot]) => {
                if (slot && slot.facultyId === facultyId) {
                    const sectionInfo = allSections.find(s => s.id === sectionId);
                    if (sectionInfo && !sectionsSet.has(sectionId)) {
                        sectionsSet.set(sectionId, sectionInfo);
                    }
                    
                    const subjectInfo = data.allSubjects.find(s => s.id === slot.subjectId);
                    const uniqueSubjectId = `${slot.subjectId}_${sectionId}`;
                    const augmentedSubject = { ...subjectInfo, id: uniqueSubjectId, name: `${subjectInfo.name} (${sectionInfo.fullDisplayName})`};
                    subjectsSet.add(augmentedSubject);

                    timetable[key] = { ...slot, sectionId: sectionId, subjectId: uniqueSubjectId };
                }
            });
        });

        return {
            baseFacultyTimetable: timetable,
            taughtSections: Array.from(sectionsSet.values()),
    ...
    const filteredTimetable = useMemo(() => {
        if (!filters.section) {
            return baseFacultyTimetable;
        }
        return Object.entries(baseFacultyTimetable).reduce((acc, [key, slot]) => {
            if (slot.sectionId === filters.section) {
                acc[key] = slot;
            }
            return acc;
        }, {});
    }, [baseFacultyTimetable, filters.section]);
    
    const augmentedData = useMemo(() => ({
        ...data,
        allSubjects: [...data.allSubjects, ...uniqueSubjects]
    }), [data, uniqueSubjects]);
            ...
            <div className="p-4 bg-gray-100 dark:bg-gray-900/50 rounded-lg mb-6">
                ...
                <div className="flex flex-col sm:flex-row items-center gap-4 w-full">
                    <div className="flex items-center gap-2 w-full sm:w-1/2">
                        <label htmlFor="class-filter" className="font-medium text-sm shrink-0">Filter by Class:</label>
                        <select
                            id="class-filter"
                        ...
                        >
                            ...
                            {taughtSections.map(sec => (
                                <option key={sec.id} value={sec.id}>{sec.fullDisplayName}</option>
                            ))}
                        </select>
                    </div>
                    <div className="flex items-center gap-2 w-full sm:w-1/2">
                        <label htmlFor="day-filter" className="font-medium text-sm shrink-0">Filter by Day:</label>
                        <select
                            id="day-filter"
                            value={filters.day}
...
export default function FacultyTimetableView() {
    ...
    const renderContent = () => {
        ...
        return <FacultyView user={mockUser} data={timetableData} />;
    };
        ...
        <div className="bg-gray-50 dark:bg-gray-900 min-h-screen font-sans text-gray-900 dark:text-gray-100 p-4 sm:p-6 lg:p-8">
            <header className="mb-8">
                <h1 className="text-2xl md:text-3xl font-bold leading-tight text-gray-900 dark:text-gray-100 flex items-center">
                    <Calendar className="mr-3 text-indigo-500" /> My Schedule
                </h1>
                <p className="text-gray-600 dark:text-gray-400">Welcome, {mockUser.details.name}.</p>
            </header>
